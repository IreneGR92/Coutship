/***********************************************
 Good-genes model of sexual selection on male courtship effort, based on 'condition-dependent handicap' mechanism as described by van Doorn & Weissing (2006).
 Males with high genetic viability pay lower marginal viability costs of displaying than do males with low genetic viability.
...
void Phenotypes()
{
	...
	for (i=0;i<NM;i++)
	{
		...
		M0[i].viab = sumViab + exp(-((k/exp(-abs(Vopt - M0[i].v)))*M0[i].t*M0[i].t) - abs(Vopt - M0[i].v)); // male viability depends on expressed trait (costs lower for HQ males) and distance from Vopt
...
//cout << "s = " << s << endl;
    ...
    for (i=0;i<SampleSize;i++)
    {
       ...
       // conditional indicator model, so attractiveness depends only on display level (not on genetic viability)
       ...
       // calculate reductions in viability for displaying males; smaller reductions for males with high genetic viability
       if (Candidates[i]==0) { dropViab = M0[Candidates[i]].viab*(1-exp((-c/exp(-abs(Vopt - M0[Candidates[i]].v)))*DisplayFlex[i]*DisplayFlex[i])); }
       else { dropViab = ( M0[Candidates[i]].viab - M0[Candidates[i]-1].viab ) * (1-exp((-c/exp(-abs(Vopt - M0[Candidates[i]].v)))*DisplayFlex[i]*DisplayFlex[i])); }
